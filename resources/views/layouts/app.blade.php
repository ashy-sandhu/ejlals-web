                        ...
                        <div class="hidden sm:flex items-center gap-2">
                            @auth
                                <div class="flex items-center gap-6">
                                    <a href="{{ route('profile.edit') }}" class="text-sm font-medium hover:text-brand-teal flex items-center gap-2 group">
                                        <div class="w-8 h-8 rounded-lg bg-brand-teal/5 flex items-center justify-center text-brand-teal group-hover:bg-brand-teal group-hover:text-white transition-all">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                        </div>
                                        <span class="hidden lg:block">My Profile</span>
                                    </a>
                        ...
                        @if (Route::has('login'))
                            @auth
                                <div class="grid grid-cols-2 gap-3">
                                    <a href="{{ route('dashboard') }}" class="flex items-center justify-center bg-brand-teal hover:bg-brand-teal/90 text-white px-5 py-3.5 rounded-xl text-sm font-bold transition-all shadow-sm">
                                        My Horizon
                                    </a>
                                    <form action="{{ route('logout') }}" method="POST" class="w-full">
                                        @csrf
                                        <button type="submit" class="w-full flex items-center justify-center border-2 border-slate-100 text-slate-500 hover:bg-rose-50 hover:text-rose-500 hover:border-rose-100 px-5 py-3 rounded-xl text-sm font-bold transition-all bg-white">
                                            Logout
                                        </button>
                                    </form>
                                </div>
