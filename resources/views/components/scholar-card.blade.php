    ...
    <div class="px-3 py-2 flex flex-col grow">
        ...
        <div class="flex items-center justify-between gap-2 mb-2.5">
            <div class="flex items-center gap-1.5 min-w-0">
                <a href="{{ route('scholars.show', $scholar->slug) }}" class="text-base font-bold text-slate-900 hover:text-brand-teal transition-colors tracking-tight truncate">
                    {{ $scholar->name }}
                </a>
            </div>
            <div class="flex items-center gap-1 shrink-0">
                <span class="material-symbols-outlined text-brand-gold !text-[14px]" style="font-variation-settings: 'FILL' 1;">star</span>
                <span class="text-[14px] font-bold text-slate-900">{{ number_format($scholar->rating ?? 5.0, 1) }}</span>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between gap-2 mb-2">
            @if($scholar->title)
            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-brand-teal/10 text-brand-teal border border-brand-teal/10 whitespace-nowrap">
                {{ $scholar->title }}
            </span>
            @else
            <div></div>
            ...
            <div class="flex items-center gap-1 text-[9px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
                <span class="material-symbols-outlined !text-[14px] leading-none" style="font-variation-settings: 'FILL' 1;">verified</span>
                <span class="uppercase tracking-wider">Ejlals Tested</span>
            </div>
            @endif
        </div>
        
        <!-- Line 3: Location Only -->
        <div class="flex items-center gap-1.5 text-slate-500 text-xs mb-2 min-w-0">
            <span class="material-symbols-outlined !text-[14px] shrink-0 text-slate-400">location_on</span>
            <span class="truncate font-normal">{{ $scholar->location ?? 'Lahore, Pakistan' }}</span>
        </div>
        ...
        <div class="flex items-center gap-2 mb-2 w-full overflow-hidden">
            ...
            @foreach($displayTags as $subject)
                <span class="px-2 py-0.5 bg-brand-gold/10 text-brand-gold/90 rounded-full text-[10px] font-medium whitespace-nowrap uppercase tracking-wider border border-brand-gold/10">
                    {{ $subject }}
                </span>
            @endforeach
        </div>
        ...
        <div class="mt-auto pt-2 border-t border-slate-100 flex items-center justify-between gap-3">
